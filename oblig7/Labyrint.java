...
public class Labyrint {
    ...
    private static void settNaboer(Labyrint l, Rute[][] brett, int yCount,
            int xCount) {
    ...
    private static void sjekkNaboer(Labyrint l, Rute[][] brett, int y, int x,
            int yCount, int xCount) {
        brett[y][x].ref(l);
    ...
    public ArrayList<ArrayList<Tuppel>> finnUtveiFra(int x, int y) {
        utveier = new Monitor();
        this.brett[y][x].finnUtvei();
        ArrayList<ArrayList<Tuppel>> res = utveier.hentRuter();
        resettBesoekt(this.brett);
        return res;
    }

    private void resettBesoekt(Rute[][] brett) {
        for (int y = 0; y < brett.length; y++)
            for(int x = 0; x < brett[0].length; x++)
                if (brett[y][x] instanceof HvitRute)
                    brett[y][x].resettBesoekt();
    }
    ...
    public Rute[][] hentBrett() { return this.brett; }
